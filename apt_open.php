...
<body>
	...
	$url2=array(
		array(
			'https://hogangnono.com/api/apt/3OD6e/trade?areaNo=8&tradeType=0&page=', 'https://hogangnono.com/api/apt/3OD6e/trade?areaNo=9&tradeType=0&page='),
		array(
			'https://hogangnono.com/api/apt/3OE97/trade?areaNo=8&tradeType=0&page=', 'https://hogangnono.com/api/apt/3OE97/trade?areaNo=9&tradeType=0&page='),
		array(
			'https://hogangnono.com/api/apt/3Or2b/trade?areaNo=5&tradeType=0&page=', 'https://hogangnono.com/api/apt/3Or2b/trade?areaNo=6&tradeType=0&page='),
		array(
			'https://hogangnono.com/api/apt/3Ot3d/trade?areaNo=5&tradeType=0&page=', 'https://hogangnono.com/api/apt/3Ot3d/trade?areaNo=8&tradeType=0&page='),
		array(
			'https://hogangnono.com/api/apt/3Ou9b/trade?areaNo=5&tradeType=0&page='),
		array(
			'https://hogangnono.com/api/apt/3Ov4b/trade?areaNo=9&tradeType=0&page=',
			'https://hogangnono.com/api/apt/3Ov4b/trade?areaNo=11&tradeType=0&page=', 'https://hogangnono.com/api/apt/3Ov4b/trade?areaNo=12&tradeType=0&page=')); //호갱노노
...
<table border=0>
	<tr>
		<td style='text-align:center; height:40px; background-color:#b33951;'><b>$apt_name[$n]</b> <input id='no_apt_list[{$n}]' type=text style='width: 50; font-size:15px; font-weight:bold; text-align: left; border:solid 0px black; background-color:#b33951;'></td>
	</tr>
</table>
<table border=0>
		");
	...
	<tr>
		...
		$apt_list=array();
		for ($k=0; $k<sizeof($url2[$n]); $k++){ //호갱노노
			$page=1;
			$isEnd=0;
			while ($isEnd<1){
				$url_hogang=$url2[$n][$k].$page;
				curl_setopt($curlsession, CURLOPT_URL, $url_hogang);
				$result = curl_exec($curlsession);
				$session = json_decode($result, true);
				$apt_list=array_merge($apt_list, $session["data"]["data"]);
				$isEnd=$session["data"]["isEnd"];
				$page++;
			}
		}
		$no_apt_list=$no_apt_list+sizeof($apt_list);

		foreach ($apt_list as $apt_list_sub){
			$apt_dong=$apt_list_sub['dong']."동";
			$apt_type=str_replace("타입", "", $apt_list_sub['area_type']);
			if(substr($apt_list_sub['price'], -4)=="0000"){
				$apt_money_after="";
			} else {
				$apt_money_after=" ".substr($apt_list_sub['price'], -4);
			}
			$apt_money=substr($apt_list_sub['price'], 0, -4)."억".$apt_money_after;
			$apt_date=substr($apt_list_sub['date_updated'], 2, 2).".".substr($apt_list_sub['date_updated'], 5, 2).".".substr($apt_list_sub['date_updated'], 8, 2);

			$date=str_replace(".", "-", $apt_date);
			$start = new DateTime($date); 
			$days = round(($end->format('U') - $start->format('U')) / (60*60*24)); 
			if($days<3) $class="content_td0"; else $class="content_td1";
			...
			echo ("
	<tr>
		<td class=$class onClick=\"location.href='https://hogangnono.com/apt/{$apt_list_sub['aptId']}/0/{$apt_list_sub['areaInfo']['no']}';\" style='cursor:pointer;'>
			<table border=0>
				<tr>				
					<td width=60 style='text-align:center;'><b>$apt_dong</b></td>
					<td width=40>$apt_type</td>
					<td width=50>$apt_list_sub[floor]</td>
					<td style='text-align:center;'>$apt_money</td>
					<td width=80 style='text-align:center;'><i>$apt_date</i></td>
				</tr>
			</table>
			<table border=0>
				<tr>				
					<td width=60 style='text-align:center; vertical-align:top;'>HG</td>
					<td width=40 style='font-size:13px; vertical-align:top;'>$apt_list_sub[direction]</td>
					<td style='font-size:13px;'>{$apt_list_sub['author']['name']} $apt_list_sub[description]</td>
				</tr>
			</table>
		</td>
	</tr>
...
<Script>document.getElementById('no_apt_list[{$n}]').value='($no_apt_list)';</script>
<br><br>
		");
	}
